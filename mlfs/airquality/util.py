...
def get_hourly_weather_forecast(city, latitude, longitude):
    ...
    hourly_dewpoint_2m = hourly.Variables(2).ValuesAsNumpy()
    ...
    hourly_data["apparent_temperature_mean"] = hourly_apparent_temperature
    hourly_data["dewpoint_2m_mean"] = hourly_dewpoint_2m
    ...
    hourly_data["shortwave_radiation_sum"] = hourly_shortwave_radiation
...
def backfill_predictions_for_monitoring(weather_fg, air_quality_df, monitor_fg, model):
    ...
    df['predicted_pm25'] = model.predict(df[['lag_1', 'lag_2', 'lag_3', "temperature_2m_mean", "apparent_temperature_mean", "dewpoint_2m_mean", "precipitation_sum", "rain_sum", "snowfall_sum", "wind_speed_10m_max", "wind_direction_10m_dominant", "shortwave_radiation_sum", "et0_fao_evapotranspiration"]])
...
def backfill_predictions_for_monitoring(weather_fg, air_quality_df, monitor_fg, model):
    ...
    # Create lags from historical data
    lag_df = air_quality_df[['date', 'pm25']].copy()
    lag_df['date'] = pd.to_datetime(lag_df['date'])
    lag_df = lag_df.sort_values(by='date')
    

    lag_df['lag_1'] = lag_df['pm25'].shift(1)
    lag_df['lag_2'] = lag_df['pm25'].shift(2)
    lag_df['lag_3'] = lag_df['pm25'].shift(3)

    # Merge weather and lag features
    features_df = pd.merge(
        features_df, 
        lag_df[['date', 'lag_1', 'lag_2', 'lag_3']], 
        on='date', 
        how='left'
    )
    features_df.fillna(0, inplace=True)

    # 4. --- THIS IS THE NEW "GENERAL" PART ---
    # Get the exact feature list the model was trained on.
    # This avoids the hard-coded list and adapts to the model.
    try:
        # For scikit-learn compatible XGBoost models
        feature_columns = model.feature_names_in_
    except AttributeError:
        # For base XGBoost models
        feature_columns = model.get_booster().feature_names

    # Check if we have all the features the model needs
    missing_features = set(feature_columns) - set(features_df.columns)
    if missing_features:
        raise ValueError(f"Error: The data is missing features the model needs: {missing_features}")

    # 5. Predict using the model's required features
    # features_df[feature_columns] ensures the correct selection and order
    features_df['predicted_pm25'] = model.predict(features_df[feature_columns])

    # 6. Continue with the rest of the original function
    df = pd.merge(features_df, air_quality_df[['date','pm25','street','country']], on="date")
    df['days_before_forecast_day'] = 1
    hindcast_df = df
    
    df_to_insert = df.drop('pm25', axis=1)
    
    # Only insert rows where we had valid lags
    # (assuming lag_1 is a good proxy for a valid row)
    if 'lag_1' in df_to_insert.columns:
         df_to_insert = df_to_insert[df_to_insert['lag_1'] != 0]
